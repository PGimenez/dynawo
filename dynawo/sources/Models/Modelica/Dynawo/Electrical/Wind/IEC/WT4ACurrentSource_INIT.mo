...
equation
  if MqG == 0 then
    ...
    XWT0Pu = -Q0Pu * SystemBase.SnRef / SNom;
  end if;
  if QlConst then
    QMax0Pu = QMaxPu;
    QMin0Pu = QMinPu;
  ...
  IqMin0Pu = gain.y;
  if IqMin0Pu + 1e-5 < min3.y then
    IqMax0Pu = min3.y;
  else
    IqMax0Pu = IqMin0Pu + 1e-5;
  end if;
  ...
  connect(min1.y, gain.u) annotation(
    Line(points = {{162, 180}, {178, 180}}, color = {0, 0, 127}));
  ...
  connect(min1.y, min3.u1) annotation(
    Line(points = {{162, 180}, {170, 180}, {170, 26}, {178, 26}}, color = {0, 0, 127}));
  connect(gain1.y, min3.u2) annotation(
    Line(points = {{141, 20}, {170, 20}, {170, 14}, {178, 14}}, color = {0, 0, 127}));
  connect(feedback1.y, gain1.u) annotation(
    Line(points = {{79, 20}, {118, 20}}, color = {0, 0, 127}));
  connect(const4.y, feedback1.u1) annotation(
    Line(points = {{41, 20}, {62, 20}}, color = {0, 0, 127}));
  ...
  connect(const5.y, feedback1.u2) annotation(
    Line(points = {{62, 60}, {70, 60}, {70, 28}}, color = {0, 0, 127}));
