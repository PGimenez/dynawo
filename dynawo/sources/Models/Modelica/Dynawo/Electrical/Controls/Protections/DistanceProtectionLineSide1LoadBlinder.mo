...
model DistanceProtectionLineSide1LoadBlinder "Simple triangular distance relay, disconnects side 1 of the protected line when the apparent impedance in within a protected zone for longer than a set duration"
  /*
...
public
  ...
  parameter Real BlinderAngle "Load angle of the load blinder (in rad)";
  parameter Real BlinderReach "Reach in the Z plane of the load blinder in pu (base UNom, SNom)";
...
equation
  if lineState.value == 2 or lineState.value == 3 and running.value then  // Can only read PMonitoredPu and QMonitoredPu if the line is connected on (at least) side1
  ...
  Blinded = if ComplexMath.'abs'(Z) > BlinderReach and abs(Modelica.ComplexMath.arg(Z)) < BlinderAngle then true else false;
  ...
  */
  ...
  when (Z.re <= R1 and Z.im <= X1 and Z.re > -Z.im) and not Blinded then
    tThresholdReached1 = time;
    Timeline.logEvent1(TimelineKeys.Zone1Arming);
  elsewhen not (Z.re <= R1 and Z.im <= X1 and Z.re > -Z.im and not Blinded) and pre(tThresholdReached1) <> Constants.inf then
    tThresholdReached1 = Constants.inf;
    Timeline.logEvent1(TimelineKeys.Zone1Disarming);
  end when;

  // Impedance comparison with the zone 2
  when (Z.re <= R2 and Z.im <= X2 and Z.re > -Z.im) and not Blinded then
    tThresholdReached2 = time;
    Timeline.logEvent1(TimelineKeys.Zone2Arming);
  elsewhen not (Z.re <= R2 and Z.im <= X2 and Z.re > -Z.im and not Blinded) and pre(tThresholdReached2) <> Constants.inf then
    tThresholdReached2 = Constants.inf;
    Timeline.logEvent1(TimelineKeys.Zone2Disarming);
  end when;

  // Impedance comparison with the zone 3
  when (Z.re <= R3 and Z.im <= X3 and Z.re > -Z.im) and not Blinded then
    tThresholdReached3 = time;
    Timeline.logEvent1(TimelineKeys.Zone3Arming);
  elsewhen not (Z.re <= R3 and Z.im <= X3 and Z.re > -Z.im and not Blinded) and pre(tThresholdReached3) <> Constants.inf then
    tThresholdReached3 = Constants.inf;
    Timeline.logEvent1(TimelineKeys.Zone3Disarming);
  end when;

  // Impedance comparison with the zone 4
  when (Z.re <= R4 and Z.im <= X4 and Z.re > -Z.im) and not Blinded then
    tThresholdReached4 = time;
    Timeline.logEvent1(TimelineKeys.Zone4Arming);
  elsewhen not (Z.re <= R4 and Z.im <= X4 and Z.re > -Z.im and not Blinded) and pre(tThresholdReached4) <> Constants.inf then
...
end DistanceProtectionLineSide1LoadBlinder;
